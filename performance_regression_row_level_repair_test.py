...
class PerformanceRegressionRowLevelRepairTest(ClusterTester):
    ...
    def _update_cl_in_stress_cmd(self, str_stress_cmd, consistency_level):
        ...
        self.log.debug(
            f"Could not find a 'cl' parameter in stress command: {str_stress_cmd}"
        )
    ...
    def preload_data(self, consistency_level=None):
        if prepare_write_cmd := self.params.get('prepare_write_cmd'):
            ...
            if not isinstance(prepare_write_cmd, six.string_types) and len(prepare_write_cmd) > 1:
                ...
                stress_cmd = (
                    self._update_cl_in_stress_cmd(
                        str_stress_cmd=prepare_write_cmd,
                        consistency_level=consistency_level,
                    )
                    if consistency_level
                    else prepare_write_cmd
                )
    ...
    def _wait_no_compactions_running(self):
        ...
        self.log.debug(msg=f"scylla_compaction_manager_compactions: {results}")
        # if all are zeros the result will be False, otherwise there are still compactions
        if results:
            assert not any(
                float(v[1]) for v in results[0]["values"]
    ...
    def test_row_level_repair_single_node_diff(self):
        ...
        for node in self.db_cluster.nodes:
            ...
            self.log.debug(
                f"Node {node.public_ip_address} initial used capacity is: {used_capacity}"
            )
        ...
        self.log.info(
            f'Repair (0% synced) time on node: {node1.name} is: {repair_time}'
        )
        ...
        repair_time = self._run_repair(node=node1)[0]  # pylint: disable=unsubscriptable-object
        self.log.info(
            f'Repair (100% synced) time on node: {node1.name} is: {repair_time}'
        )
    ...
    def test_row_level_repair_3_nodes_small_diff(self):
        ...
        self.log.info(
            f'Repair (99.8% synced) time on node: {node3.name} is: {repair_time}'
        )
    ...
    def test_row_level_repair_large_partitions(self):
        ...
        self.log.info(
            f'Repair (with large partitions) time on node: {node3.name} is: {repair_time}'
        )
    ...
    def test_row_level_repair_during_load(self, preload_data=True):
        ...
        self.log.info(
            f'Repair (during r/w load) time on node: {node1.name} is: {repair_time}'
        )
